...
| §3.1 | Performance "budgeted and CI-enforced" | Contradicted | `.github/workflows/ci.yml` has no perf/benchmark job; no criterion benches in the workspace | ◑ **Partly addressed** · there is now a criterion bench (`lumit-project/benches/document_scale`, S2–S5 on the stress fixture) — so "no criterion benches" no longer holds. Still no perf *job* in CI enforcing budgets; that gate + the render-side B-benches wait on the pixel-pass migration |
...
| 5.2/9.2 | "Convert to rate" menu + drift warning badge | Partial (unwired) | Core fit implemented (`retime.rs:1041,1462`); no menu, no badge | ◑ Partial · ✅ CI-green (macOS built lumit-ui with the change) · wired a **→Rate** button in the speed-lens graph header that calls the tested `with_segment_as_rate` on the segment under the playhead and commits `SetLayerRetime`; the fit **drift** is surfaced as a status notice (e.g. "fitted, 3 ms drift") rather than a persistent badge. The always-visible drift badge is still to build · 👁 try →Rate on a mapped segment |
...
| §1.1 | `FrameSource`/`KernelExecutor`/`CacheStore` trait seams in eval | Not implemented | Only `SourceStamper` exists; the pixel pass those traits serve is unbuilt (`graph.rs:3-4`) | ✅ Done · **built** as `lumit-eval::exec`: the three trait seams + a demand-pull executor walking the compiled graph (shared-source dedup, cache get/put by content key, epoch cancellation, malformed-graph guards) — 7 unit tests against fakes, green locally. Real GPU/media/cache implementations still to wire app-side |
...
| §2 | Work-stealing worker pool (cores−1) | Not implemented | No pool; ad-hoc `thread::spawn` per job in `lumit-ui` | ✅ Done (component) · **built** as `lumit-eval::pool`: dedicated rayon pool (cores−3 min 2 per the impl note — 05 §2 synced from its stale cores−1), bounded two-class queues, interactive-first at job boundaries, panic containment — 5 unit tests green locally. **Correction to the original evidence:** the shell's existing spawns are all decode/IO/service roles (audio decode, mixdown, probe, the persistent PreviewEngine loop, export worker) which 05 §2 deliberately keeps *off* the pool — they were never pool tenants. The pool's consumers (per-frame render jobs) arrive with the pixel-pass switch |
...
| § | Claim | Status | Evidence / what's missing | Resolution / next step |
|---|---|---|---|---|
| 1.1 | Comp lowered to an eval-graph that is demand-pulled to pixels | Partial | DAG compiles but the pixel pass is deferred (`graph.rs:3`); real renderer is the imperative draw-list in `export.rs:1120` / preview shell | ◑ Executor built + **real-pixel proof** · `lumit-eval::exec::render_frame` demand-pulls the graph through the trait seams (tested against fakes, incl. pool-hosted render), and the walking-skeleton test (`lumit-gpu/tests/exec_skeleton.rs`, lavapipe-verified locally) drives the **real compositor** through the sockets. Vocabulary now proven through the seams: solids, exact transform placement, per-layer opacity, linear-light blend, zero-GPU-work cache hit, **all blend modes**, **layer masks**, and **adjustment-layer effects** (7 lavapipe tests). Remaining: the Retime slice (needs frame interpolation/flow, belongs with the migration) + switching preview/export onto the executor |
...
| 3.5a | Luma mattes use Rec.709 Y of the **sRGB-encoded** signal | **Contradicted** | Luma dot-product runs on linear premultiplied values (`composite.wgsl:214,252`) — luma mattes will read differently than specified | ✅ Done · 22a9ddd · tested · ✅ owner-verified on real footage |
...
| 5.3 | Cost-aware GreedyDual eviction, pinning, demotion | Not implemented | Plain LRU (RAM) and oldest-mtime (disk) | ✅ **Built (RAM tier)** · `lumit-cache::ByteLru` now evicts by the spec's "stale × size ÷ recompute-cost" rule (reduces to LRU for equal size/cost, so existing callers are unchanged in behaviour and API); `pin`/`unpin` protect the displayed frame + playhead window, with bounded overage when only pins remain; cost hints via `insert_with_cost`. 19 tests green locally. Remaining: caller adoption (real cost hints, pinning the playhead window) + VRAM→RAM demotion (waits on the VRAM tier) |
...
| 3.3 | Proxy badges/toggle; missing-footage badge + relink flow | Not implemented | Only an "unreadable: {e}" line (`panels.rs:483-488`) | ◑ **Resolver engine built** · the relink *logic* now exists (`lumit-project::resolve_media` + sibling remapping, see 10 §2) so the missing-footage flow has a backend. Remaining (lumit-ui): the missing-footage badge, the relink dialogue that drives `resolve_media`, and proxy badges/toggle (proxies unbuilt) |
...
| 15 | Remappable keymap + conflict detection + AE preset; keymap settings page | Not implemented | Hardcoded bindings; no keymap page | ◑ **Core built** · new `lumit-keymap` crate (pure logic): `Chord` (parse/display, order+case-insensitive, Ctrl/Cmd/Mod primary), `KeyContext`, `ActionId`, `Keymap` with context-precedence lookup, conflict detection (Global-overlap rule), bind/unbind/search, shareable serde file. The **full §15 default table** ships (Global/Tools/Timeline/Graph/Viewer/Panels, conflict-free) + an AE preset. 8 tests, ~96% covered (in the coverage gate). Remaining: wiring the lumit-ui key-event path + a Settings → Keymap page onto it |
...
| 1 | Newer reader **migrates** older files | Not implemented | Only refuse-too-new gating (`lib.rs:114`); no migration step | ✅ **Built** · `lumit-project` schema-migration framework: an ordered `Migration` chain transforms raw `project.json` (as serde `Value`, before typing) version→version; `open()` walks an older file up to the current schema, current files take the direct byte-identical path. Bounded (never loops on a malformed chain). Chain empty today (0.1.0 is first); each schema bump appends a step. 3 tests, green locally |
...
| 2 | Fingerprint + 4-step relink + sibling auto-relink | Partial / not implemented | No fingerprint on `MediaRef`; no relink resolver | ✅ **Built (engine)** · `MediaRef.fingerprint` added (size + mtime + blake3 head/tail hash, optional + skipped when unset so old projects round-trip byte-identical); `lumit-project::fingerprint_path` computes it in ≤2×64 KiB reads (cheap on huge footage). `resolve_media` does the 4 steps (relative → absolute → fingerprint search of search-roots + project tree → Missing); `path_mapping`/`apply_mapping` relink siblings that moved the same way. 8 tests green locally. Remaining: the relink-dialogue UI + calling `resolve_media` from `open()` (lumit-ui) |
| 2 | Collect-for-sharing command | Not implemented | No such function | ✅ **Built (engine)** · `lumit-project::collect_for_sharing` copies every located reference into `dest/media/`, rewrites refs project-relative with nothing machine-specific, disambiguates colliding basenames, preserves fingerprints, and reports unlocatable media instead of failing (partial collect still opens). 3 tests green locally. Remaining: the menu command + save-into-dest wiring (lumit-ui) |
...
| §7.3 | Headless benchmark harness emitting B3–B8/B11 | Not implemented | No harness, no `benches/`, no criterion | ◑ **Document-scale harness built** · `lumit-project/benches/document_scale` (criterion, default-features off) times S2–S5 on the REFERENCE stress fixture: save, open (~194 ms locally), commit, undo. Runs on demand (`cargo bench`); compiles under CI clippy but is not a gate yet. Remaining: the B3–B11 render/playback/export benches (need the pixel-pass migration) + wiring budgets into CI |
| §1/§2.1/§7.3 | Reference comp + deterministic stress fixture in repo for CI | Not implemented | No fixtures or builder | ◑ **Stress fixture built** · `lumit-project::fixtures::stress_document` builds the §2.1 document (200 comps / 5,000 layers / 250,000 keyframes / 2,000 footage) deterministically from fixed UUIDs, parameterised (`TINY` for tests, `REFERENCE` for the spec); determinism + spec-count + save/open tested. Remaining: the faithful 5-layer reference comp (§1, LUT/glow/retime) |
...
| §5 | Journal compaction story | Partial | Undo journal is an ever-growing `Vec` (`store.rs:23-24`) | ✅ **Built** · the in-memory undo/redo history is now bounded to `MAX_UNDO_DEPTH` (500): each commit past the cap drops the oldest steps (current document untouched); redo is transitively bounded; crash recovery unaffected (disk journal is separate). Compaction story documented at the type; regression-tested (lumit-core 224 tests green locally) |
