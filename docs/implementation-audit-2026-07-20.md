...
| 4.7 | `[`/`]`, Alt+[/], Ctrl+D | Not implemented | Unbound; duplicate via menu only | ✅ Done · ⚠ UI keys CI-compiled; span maths tested locally · all bound in `global_shortcuts`: **Cmd/Ctrl+D** duplicate; **`[`/`]`** move the selected layer's in/out to the playhead; **Alt+`[`/`]`** trim that edge. The AE span maths live in `lumit_core::ops::edit_layer_span` (4 unit tests, verified locally); the UI resolves selection + playhead and commits `SetLayerSpan` · 👁 try the layer-edit keys |
