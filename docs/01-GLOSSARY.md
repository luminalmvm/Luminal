...
| **Motion blur** | Shutter-simulated blur along motion. Three distinct forms: the per-layer transform **motion-blur switch** (a layer switch, K-120, smears one layer along its own transform); the **Motion blur** effect (accumulation, docs/08 §3.26 — re-renders the whole scene below at sub-frame times and averages, the correct whole-scene kind); and the **Fast motion blur** effect (optical flow, §3.2 — a single-pass per-pixel smear of footage-internal motion). |
