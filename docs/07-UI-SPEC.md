...
Cache root folder added K-116):** the Settings window exists — a macOS-System-Settings-style
...
here out of the Window menu, K-092), **Performance** (RAM frame-cache budget, disk-cache cap
and VRAM frame-cache budget, all applied live, a Clear cache action that empties the RAM and
...
beside the project file), and **General** (reset workspace, an **Autosave** group — interval in
minutes and copies kept, defaulting to the previous 5 min / 5 copies — and version). The
remaining groups (CUDA, decoder pool size, worker thread cap, proxy generation policy, Preview,
Colour, Export, Keymap, Plugins) fill in on this same surface as those systems gain their
controls.
