...
- [~] TL4 Added the top row above the ruler (`timeline_top_row`): current time (m:ss:ff + frame) top-left, a layer-search box in the middle (filters the outline by name, `timeline_layer_search`), and the graph-view toggle + MB master top-right — both moved up from the bottom bar. REMAINING: the "hide-layer" icon (ambiguous — shy layers? which layers to hide?) — needs clarification.
