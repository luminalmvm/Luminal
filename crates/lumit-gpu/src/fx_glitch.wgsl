// Glitch — Block displacement and Scanlines (docs/08-EFFECTS.md §3.12,
// schema status note; Datamosh is deferred). Mirrors
// lumit_core::fx::cpu::glitch op-for-op (§1.6: the CPU is the oracle). The
// per-block hash runs here too, not host-precomputed: the block index is a
// per-pixel quantity (there are too many blocks to fit a table into the
// uniform), so `splitmix32` — a 32-bit sibling of Shake's splitmix64
// lattice, added because WGSL has no 64-bit integer type to host the
// original — runs identical wrapping u32 ops on both sides.
...
fn glitch(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    if (p.intensity == 0.0 || (p.block_enabled == 0u && p.scanline_enabled == 0u)) {
    ...
    var sy = pos.y;
    var chan = 0.0;
    if (p.block_enabled == 1u) {
        // Grid jitter (status note): a hashed offset of the *nominal*
        // block, scaled by Intensity, decides which block a pixel reads.
        ...
        chan = (block_hash01(4u, bx, by) - 0.5) * 2.0 * p.chan * p.intensity;
        let slice_u = block_hash01(5u, bx, by);
        let slice_h_u = block_hash01(6u, bx, by);

        // Slice repeat: fold the block's own local Y to a short hashed
        // repeat height instead of a plain read.
        var eff_y = jpos.y;
    ...
    // R/B split from the block hash (alpha follows green, like RGB
    // split); always computed — chan is exactly 0.0 when block
    // displacement is off, an exact no-op offset.
    let r = bilinear(sx - chan, sy, size).r;
    let g = bilinear(sx, sy, size);
    let b = bilinear(sx + chan, sy, size).b;
    var c = vec4<f32>(r, g.g, b, g.a);

    if (p.scanline_enabled == 1u) {
        let period = max(p.period, 1.0);
        let yp = pos.y + p.roll_px;
        let cell = yp / period;
        let cell_floor = floor(cell);
        let t = cell - cell_floor;
        // WGSL's % is truncated (can be negative); folding to {0,1} via
        // abs matches Rust's rem_euclid(2) for parity purposes exactly
        // (== cpu::glitch's `(cell_floor as i64).rem_euclid(2) != 0`).
        ...
        c = vec4<f32>(c.r * eff_mult, c.g * eff_mult, c.b * eff_mult, c.a);
    }
