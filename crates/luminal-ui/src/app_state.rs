...
use luminal_core::time::{Duration, FrameRate, Rational};
use luminal_core::DocumentStore;
use luminal_project::JournalFile;
...
pub mod preview {
    ...
    impl Default for PreviewEngine {
        fn default() -> Self {
            ...
            std::thread::spawn(move || {
                let mut decoders: HashMap<Uuid, luminal_media::VideoDecoder> = HashMap::new();
                // Decoded-frame RAM cache (K-016 tier seed): recently shown
                // frames re-display instantly instead of re-decoding.
                let mut frame_cache: luminal_cache::ByteLru<
                    (Uuid, usize, Option<u32>),
                    CachedFrame,
                > = luminal_cache::ByteLru::new(512 * 1024 * 1024);
                loop {
    ...
    impl luminal_cache::ByteSized for CachedFrame {
    ...
    fn decode(
        decoders: &mut HashMap<Uuid, luminal_media::VideoDecoder>,
        cache: &mut luminal_cache::ByteLru<(Uuid, usize, Option<u32>), CachedFrame>,
        req: &Request,
        ...
        let dec = match decoders.entry(req.item) {
            ...
            std::collections::hash_map::Entry::Vacant(e) => {
                let index = luminal_media::index::build_frame_index(&req.path)
                    .map_err(|e| e.to_string())?;
                let dec = luminal_media::VideoDecoder::open(&req.path, index)
                    .map_err(|e| e.to_string())?;
                e.insert(dec)
            }
        };
...
pub mod media {
    ...
    pub enum MediaStatus {
        Probing,
        Ready {
            probe: luminal_media::MediaProbe,
            frames: usize,
            vfr: bool,
        },
        Failed(String),
    ...
    fn probe_and_index(path: &std::path::Path) -> MediaStatus {
        let probe = match luminal_media::probe::probe(path) {
            Ok(p) => p,
        ...
        let cache_dir = luminal_project::media_index_dir();
        let cached = match (&cache_dir, luminal_media::Fingerprint::of(path)) {
            (Some(dir), Ok(fp)) => luminal_media::FrameIndex::load_cached(dir, &fp),
            _ => None,
        };
        let index = match cached {
            Some(index) => index,
            None => match luminal_media::index::build_frame_index(path) {
                Ok(index) => {
...
fn centred_transform(
    ...
    comp_w: u32,
    comp_h: u32,
) -> luminal_core::model::TransformGroup {
...
#[cfg(feature = "media")]
impl luminal_cache::ByteSized for CachedCompFrame {
    fn byte_size(&self) -> usize {
        self.rgba.len() + 16
    }
}
...
#[cfg(feature = "media")]
impl luminal_eval::SourceStamper for PreviewStamper<'_> {
...
pub struct GraphSelection {
    ...
    pub prop: luminal_core::model::TransformProp,
...
impl GraphSelection {
    ...
    pub fn indices_for(&self, keys: &[luminal_core::anim::Keyframe]) -> Option<Vec<usize>> {
        self.keys
            .iter()
...
pub struct AppState {
    ...
    #[cfg(feature = "media")]
    audio_engine: Option<luminal_audio::AudioEngine>,
    #[cfg(feature = "media")]
    audio_cache: std::collections::HashMap<Uuid, std::sync::Arc<luminal_media::AudioBuffer>>,
    #[cfg(feature = "media")]
    audio_loaded: Option<Uuid>,
    #[cfg(feature = "media")]
    audio_rx: std::sync::mpsc::Receiver<(Uuid, Result<luminal_media::AudioBuffer, String>)>,
    #[cfg(feature = "media")]
    audio_tx: std::sync::mpsc::Sender<(Uuid, Result<luminal_media::AudioBuffer, String>)>,
    ...
    #[cfg(feature = "media")]
    comp_audio_rx: std::sync::mpsc::Receiver<(Uuid, luminal_media::AudioBuffer)>,
    #[cfg(feature = "media")]
    comp_audio_tx: std::sync::mpsc::Sender<(Uuid, luminal_media::AudioBuffer)>,
    ...
    pub prop_edit: Option<(Uuid, luminal_core::model::TransformProp, f64)>,
    ...
    pub pen_path: Vec<luminal_core::mask::Vertex>,
    /// Property shown in the graph editor.
    pub graph_prop: Option<luminal_core::model::TransformProp>,
    ...
    pub graph_set_interp: Option<luminal_core::anim::SideInterp>,
    ...
    #[cfg(feature = "media")]
    pub comp_frame_cache: luminal_cache::ByteLru<u128, CachedCompFrame>,
...
impl Default for AppState {
    fn default() -> Self {
        ...
        Self {
            ...
            #[cfg(feature = "media")]
            comp_frame_cache: luminal_cache::ByteLru::new(512 * 1024 * 1024),
            #[cfg(feature = "media")]
            cache_epoch: 0,
            #[cfg(feature = "media")]
            cached_present: None,
...
impl AppState {
    ...
    pub fn open_dialog(&mut self) {
        let picked = rfd::FileDialog::new()
            .add_filter("Luminal project", &["kir"])
            .pick_file();
    ...
    pub fn open_path(&mut self, path: &Path) {
        let Some((doc, _manifest)) = self.report(luminal_project::open(path)) else {
            return;
        };
    ...
    pub fn resolve_recovery(&mut self, recover: bool) {
        ...
        if recover {
            ...
            for op in &pending.ops {
                if luminal_core::ops::apply(&mut doc, op).is_err() {
                    break;
                }
                replayed += 1;
            }
    ...
    pub fn save(&mut self) {
        ...
        if self.report(luminal_project::save(&doc, &path)).is_some() {
            if let Some(journal) = &self.journal {
                let _ = journal.clear();
            }
            self.path = Some(path);
    ...
    pub fn autosave_tick(&mut self) {
            ...
            && self.last_autosave.elapsed().as_secs() >= AUTOSAVE_INTERVAL_SECS
        {
            ...
            if let Some(path) = self.path.clone() {
                let doc = self.store.snapshot();
                let _ = self.report(luminal_project::autosave(&doc, &path, AUTOSAVE_KEEP));
            }
        }
    }
    ...
    pub fn add_footage_to_comp(&mut self, item_id: Uuid) {
        use luminal_core::model::{Layer, LayerKind, Switches};
    ...
    pub fn add_text_layer(&mut self) {
        ...
        let transform = TransformGroup {
            position_x: luminal_core::anim::Property::fixed(f64::from(comp.width) * 0.5),
            position_y: luminal_core::anim::Property::fixed(f64::from(comp.height) * 0.5),
    ...
    pub fn add_camera_layer(&mut self) {
        use luminal_core::model::{Layer, LayerKind, Switches, TransformGroup};
        use luminal_core::time::CompTime;
        let Some(comp_id) = self.preview_comp.or(self.selected_comp) else {
            self.error = Some("select a composition first".into());
            return;
        ...
        let layer = Layer {
            ...
            kind: LayerKind::Camera {
                zoom: luminal_core::anim::Property::fixed(f64::from(comp.width) * 50.0 / 36.0),
            },
            in_point: CompTime(Rational::ZERO),
            out_point: CompTime(comp.duration.0),
            start_offset: CompTime(Rational::ZERO),
    ...
    pub fn add_sequence_layer(&mut self) {
        use luminal_core::model::{Layer, LayerKind, Switches};
        use luminal_core::sequence::{Clip, ClipSource};
        use luminal_core::time::CompTime;
        let Some(comp_id) = self.preview_comp.or(self.selected_comp) else {
            self.error = Some("select a composition first".into());
            return;
    ...
    pub fn convert_to_sequenced_layer(&mut self) {
        ...
        let clip = Clip {
            ...
            retime: retime
                .clone()
                .unwrap_or_else(|| luminal_core::retime::Retime::identity(dur, Rational::ZERO)),
    ...
    pub fn set_selected_clip_ramp(
        ...
        let pct = |p: f64| {
            luminal_core::Rational::from_f64_on_grid(p / 100.0, 1000)
    ...
    pub fn set_selected_clip_interp(&mut self, interp: luminal_core::retime::Interpolation) {
        use luminal_core::model::LayerKind;
        let (Some(comp_id), Some(layer_id), Some(clip_id)) =
            (self.selected_comp, self.selected_layer, self.selected_clip)
        else {
    ...
    fn ensure_auto_folder_ops(&self, kind: luminal_core::ops::AutoFolderKind) -> (Uuid, Vec<Op>) {
        use luminal_core::model::Folder;
        use luminal_core::ops::AutoFolderKind;
        let doc = self.store.snapshot();
        let slot = match kind {
    ...
    pub fn add_solid_layer(&mut self) {
        ...
        let layer = Layer {
            ...
            transform: TransformGroup {
                position_x: luminal_core::anim::Property::fixed(f64::from(comp.width) * 0.5),
                position_y: luminal_core::anim::Property::fixed(f64::from(comp.height) * 0.5),
                anchor_x: luminal_core::anim::Property::fixed(f64::from(comp.width) * 0.5),
                anchor_y: luminal_core::anim::Property::fixed(f64::from(comp.height) * 0.5),
                ..TransformGroup::default()
            },
            matte: None,
            blend: Default::default(),
    ...
    pub fn add_solid_def_layer(&mut self, def_id: Uuid) {
        ...
        let layer = Layer {
            ...
            transform: TransformGroup {
                position_x: luminal_core::anim::Property::fixed(f64::from(comp.width) * 0.5),
                position_y: luminal_core::anim::Property::fixed(f64::from(comp.height) * 0.5),
                anchor_x: luminal_core::anim::Property::fixed(f64::from(def.width) * 0.5),
                anchor_y: luminal_core::anim::Property::fixed(f64::from(def.height) * 0.5),
                ..TransformGroup::default()
            },
            matte: None,
            blend: Default::default(),
    ...
    pub fn frame_key_for(&self, comp_id: Uuid, frame: usize) -> Option<u128> {
        ...
        luminal_eval::comp_frame_key(
            &doc,
            comp,
            t,
            luminal_eval::Quality { divisor: 1 },
            &self.stamper(&doc),
        )
        .map(|k| k.0)
    }
        ...
        for layer in &comp.layers {
            ...
            match &layer.kind {
                ...
                LayerKind::Sequence { clips } => {
                    ...
                    if let Some((_id, luminal_core::sequence::ClipSource::Footage(item), st)) =
                        luminal_core::sequence::resolve(clips, lt)
                    {
                        if let (
                        ...
                        ) = (doc.item(item), self.media.map.get(&item))
                        {
                            if let Some(video) = probe.video.as_ref() {
                                use luminal_core::retime::Interpolation;
                                let interp = luminal_core::sequence::active_clip(clips, lt)
    ...
    pub fn request_preview_audio(&mut self) {
        ...
        std::thread::spawn(move || {
            let result = luminal_media::audio::decode_all(&path, rate).map_err(|e| e.to_string());
            let _ = tx.send((id, result));
        });
    }

    #[cfg(feature = "media")]
    fn ensure_audio_engine(&mut self) -> Option<&luminal_audio::AudioEngine> {
        if self.audio_engine.is_none() {
            match luminal_audio::AudioEngine::new() {
    ...
    pub fn prepare_comp_audio(&mut self, comp_id: Uuid) {
        ...
        std::thread::spawn(move || {
            ...
            let decoded: Vec<(luminal_media::AudioBuffer, f64, f64, f64)> = jobs
                .into_iter()
                .filter_map(|(path, in_s, out_s, off_s)| {
                    luminal_media::audio::decode_all(&path, rate)
                        .ok()
                        .map(|buf| (buf, in_s, out_s, off_s))
                })
                .collect();
            let total_frames = (duration_s * f64::from(rate)).round().max(0.0) as usize;
            let placements: Vec<luminal_audio::mix::PlacedAudio> = decoded
                .iter()
                .filter_map(|(buf, in_s, out_s, off_s)| {
                    let (start_frame, src_start, len) = luminal_audio::mix::place_on_timeline(
                        *in_s,
                        *out_s,
                        *off_s,
                        buf.samples.len() / 2,
                    ...
                    Some(luminal_audio::mix::PlacedAudio {
                        start_frame,
                ...
                .collect();
            let samples = luminal_audio::mix::mix_stereo(&placements, total_frames);
            let _ = tx.send((comp_id, luminal_media::AudioBuffer { rate, samples }));
        });
    }
    ...
    pub fn poll_comp_audio(&mut self) {
        ...
        self.comp_waveform = Some((
            comp_id,
            luminal_audio::mix::waveform_peaks(&buffer.samples, 2048),
        ));
    ...
    pub fn add_marker_at_playhead(&mut self) {
        ...
        markers.push(luminal_core::markers::Marker::user(
            uuid::Uuid::now_v7(),
            t.0,
        ));
        markers.sort_by_key(|m| m.time.0);
        self.commit(luminal_core::Op::SetCompMarkers {
            comp: comp_id,
            markers,
        });
    }
    ...
    pub fn detect_beats(&mut self, comp_id: Uuid, sensitivity: f32) {
        ...
        std::thread::spawn(move || {
            ...
            let placements: Vec<luminal_audio::mix::PlacedAudio> = decoded
                ...
                .collect();
            let samples = luminal_audio::mix::mix_stereo(&placements, total_frames);
            let analysis = luminal_audio::beat::analyse_stereo(&samples, rate, sensitivity);
            ...
            let snapped = luminal_audio::beat::snap_to_grid(&times, analysis.bpm, 0.045);
    ...
    pub fn poll_beats(&mut self) {
        ...
        let new_beats: Vec<luminal_core::markers::Marker> = beats
            .iter()
            .filter_map(|(t, c)| {
                let time = luminal_core::Rational::from_f64_on_grid(t.max(0.0), 1000).ok()?;
                Some(luminal_core::markers::Marker::beat(
                    uuid::Uuid::now_v7(),
                    time,
                    *c,
                ))
            ...
            .collect();
        let markers = luminal_core::markers::with_regenerated_beats(&comp.markers, new_beats);
        self.commit(luminal_core::Op::SetCompMarkers {
            comp: comp_id,
            markers,
        });
...
mod preview_tests {
    ...
    use luminal_media::index::tests_support::fixture;
