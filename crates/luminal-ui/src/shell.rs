...
use luminal_core::model::ProjectItem;
...
/// Bridges the tiling tree to Luminal's panels and house styling.
...
fn ease_label(e: luminal_core::retime::Ease) -> &'static str {
    use luminal_core::retime::Ease;
    match e {
        Ease::Linear => "Linear",
        Ease::Slow => "Slow",
        Ease::Fast => "Fast",
...
fn moved_span(
    in_point: luminal_core::time::CompTime,
    out_point: luminal_core::time::CompTime,
    start_offset: luminal_core::time::CompTime,
    delta: f64,
) -> (
    luminal_core::time::CompTime,
    luminal_core::time::CompTime,
    luminal_core::time::CompTime,
) {
    let shift = |t: luminal_core::time::CompTime| {
        luminal_core::time::CompTime(rational_at(t.0.to_f64() + delta))
    };
    ...
    for action in actions {
        match action {
            ...
            PanelAction::Delete(id) => {
                app.commit(luminal_core::Op::RemoveItem { id });
...
fn project_header(
    ...
    theme: &Theme,
    app: &AppState,
    doc: &luminal_core::model::Document,
...
#[allow(dead_code)]
fn layer_type_style(kind: &luminal_core::model::LayerKind, theme: &Theme) -> (Icon, egui::Color32) {
    use luminal_core::model::LayerKind;
    match kind {
...
fn timeline_panel(ui: &mut egui::Ui, theme: &Theme, app: &mut AppState) {
    ...
    let mut pending: Option<luminal_core::Op> = None;
    // A per-clip speed-ramp edit (start %, end %, ease), applied after the loop.
    let mut clip_ramp_edit: Option<(f64, f64, luminal_core::retime::Ease)> = None;
    // A per-clip frame-interpolation edit, applied after the layer loop.
    let mut clip_interp_edit: Option<luminal_core::retime::Interpolation> = None;
    ...
    for m in &comp.markers {
        ...
        let (col, top) = match m.kind {
            luminal_core::markers::MarkerKind::Beat { confidence } => (
                theme
                    .accent
    ...
    if ruler_resp.clicked() || ruler_resp.dragged() {
        if let Some(pos) = ruler_resp.interact_pointer_pos() {
            ...
            let secs = luminal_core::markers::snap_time(
                rational_at(raw),
                &comp.markers,
    ...
    egui::ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            ...
            for layer in &comp.layers {
                ...
                let mut ctx_op: Option<luminal_core::Op> = None;
                ...
                row_resp.context_menu(|ui| {
                    ui.menu_button("Add mask", |ui| {
                        ...
                        if ui.button("Rectangle").clicked() {
                            new_mask = Some(luminal_core::mask::Mask::rectangle(
                                w * 0.25,
                                h * 0.25,
                                w * 0.5,
                                h * 0.5,
                        ...
                        if ui.button("Ellipse").clicked() {
                            new_mask = Some(luminal_core::mask::Mask::ellipse(
                                w * 0.5,
                                h * 0.5,
                                w * 0.3,
                                h * 0.3,
                        ...
                        if ui.button("Star").clicked() {
                            new_mask = Some(luminal_core::mask::Mask::star(
                                w * 0.5,
                                h * 0.5,
                                w * 0.32,
                                w * 0.14,
                        ...
                        if let Some(m) = new_mask {
                            ...
                            masks.push(m);
                            ctx_op = Some(luminal_core::Op::SetLayerMasks {
                                comp: comp_id,
                                layer: layer.id,
                                masks,
                            });
                    ...
                    if matches!(layer.kind, luminal_core::model::LayerKind::Footage { .. })
                    ...
                    if matches!(
                        layer.kind,
                        luminal_core::model::LayerKind::Footage {
                            retime: Some(_),
                            ..
                        }
                ...
                let is_footage =
                    matches!(layer.kind, luminal_core::model::LayerKind::Footage { .. });
                ...
                if !app.timeline_graph_mode {
                    ...
                    let move_dx = match app.move_edit {
                        Some((id, raw_in)) if id == layer.id => {
                            ...
                            let snapped = luminal_core::markers::snap_time(
                    ...
                    #[cfg(feature = "media")]
                    if let luminal_core::model::LayerKind::Footage {
                        item,
                        retime: Some(rt),
                    } = &layer.kind
                    {
                    ...
                    if let luminal_core::model::LayerKind::Sequence { clips } = &layer.kind {
                        let off = layer.start_offset.0.to_f64();
                        for clip in clips {
                            ...
                            if let Some((id, is_out, secs)) = app.trim_edit.take() {
                                if id == layer.id && is_out == out_edge {
                                    ...
                                    if new_out - new_in >= min_len {
                                        pending = Some(luminal_core::Op::SetLayerSpan {
                                            comp: comp_id,
                                            layer: layer.id,
                                            in_point: luminal_core::time::CompTime(rational_at(
                                                new_in,
                                            )),
                                            out_point: luminal_core::time::CompTime(rational_at(
                                                new_out,
                                            )),
                    ...
                    if bar_visible
                        && !matches!(layer.kind, luminal_core::model::LayerKind::Sequence { .. })
                    {
                ...
                if expanded {
                    ...
                    ui.scope(|ui| {
                                ...
                                ui.horizontal_wrapped(|ui| {
                                    ...
                                    for (mi, mask) in layer.masks.iter().enumerate() {
                                            ...
                                            .clicked()
                                        {
                                            ...
                                            pending = Some(luminal_core::Op::SetLayerMasks {
                                                comp: comp_id,
                                                layer: layer.id,
                                                masks,
                                            });
                        ...
                        if let luminal_core::model::LayerKind::Text { document } = &layer.kind {
                            ui.indent(("text", layer.id), |ui| {
                                ui.horizontal(|ui| {
                                            ...
                                            && (size - document.size).abs() > f64::EPSILON
                                    {
                                        ...
                                        doc_new.text = text;
                                        doc_new.size = size;
                                        pending = Some(luminal_core::Op::SetTextDocument {
                                            comp: comp_id,
                                            layer: layer.id,
                                            document: doc_new,
                                        });
                        ...
                        if let luminal_core::model::LayerKind::Camera { zoom } = &layer.kind {
                                ...
                                ui.horizontal(|ui| {
                                    ...
                                    if resp.drag_stopped() || resp.lost_focus() {
                                        if (value - committed).abs() > f64::EPSILON {
                                            ...
                                            pending = Some(luminal_core::Op::SetCameraZoom {
                                                comp: comp_id,
                                                layer: layer.id,
                                                animation,
                                            });
                            ...
                            if let Some(cid) = app.selected_clip {
                                if let Some(clip) = clips.iter().find(|c| c.id == cid) {
                                    ui.indent(("clipspeed", layer.id), |ui| {
                                        ...
                                        ui.horizontal(|ui| {
                                            use luminal_core::retime::{FlowParams, Interpolation};
                                            ui.label(
    ...
    if app.timeline_graph_mode {
        ...
        if !retime && app.graph_prop.is_some() {
            ...
            if zc
                ...
                .clicked()
            {
                app.graph_set_interp = Some(luminal_core::anim::SideInterp::Linear);
            }
            if zc
                .small_button("Bezier")
                ...
                .clicked()
            {
                app.graph_set_interp = Some(luminal_core::anim::EASY_EASE);
            }
        }
    }
...
#[cfg(feature = "media")]
impl LayerMap {
    fn of(layer: &luminal_core::model::Layer, lt: f64, draw: egui::Rect, scale: f32) -> Self {
...
fn mask_overlay(
    ...
    let mut committed: Option<Vec<luminal_core::mask::Mask>> = None;
    for (mi, mask) in layer.masks.iter().enumerate() {
    ...
    if let Some(masks) = committed {
        app.commit(luminal_core::Op::SetLayerMasks {
            comp: comp_id,
            layer: layer_id,
            masks,
        });
...
fn anchor_overlay(
    ...
    if layer.switches.three_d || matches!(layer.kind, luminal_core::model::LayerKind::Camera { .. })
    {
    ...
    if resp.drag_stopped() {
        if let Some(new_anchor) = app.origin_drag.take() {
            ...
            app.commit(luminal_core::Op::Batch { ops });
            app.refresh_preview();
        }
    }
        ...
        if view.drag_stopped() {
            ...
            if w > 2.0 && h > 2.0 {
                let mask = match app.shape_kind {
                    ShapeKind::Rectangle => luminal_core::mask::Mask::rectangle(x0, y0, w, h),
                    ShapeKind::Ellipse => luminal_core::mask::Mask::ellipse(
                        x0 + w * 0.5,
                        y0 + h * 0.5,
                        w * 0.5,
                        h * 0.5,
...
fn pen_overlay(
    ...
    if view.clicked() {
        if let Some(pos) = view.interact_pointer_pos() {
            ...
            if close_hit {
                let mut masks = layer.masks.clone();
                masks.push(luminal_core::mask::Mask {
                    id: uuid::Uuid::now_v7(),
                    name: format!("Path {}", masks.len() + 1),
                    path: luminal_core::mask::BezierPath {
                ...
                app.pen_path.push(luminal_core::mask::Vertex {
...
fn follow_edit(app: &mut AppState, op: &luminal_core::Op) {
    match op {
        luminal_core::Op::SetTransformProperty { layer, prop, .. } => {
            app.selected_layer = Some(*layer);
            app.graph_prop = Some(*prop);
            app.graph_retime = false;
        }
        luminal_core::Op::SetLayerRetime { layer, .. } => {
            app.selected_layer = Some(*layer);
            app.graph_retime = true;
        }
        luminal_core::Op::Batch { ops } => {
...
fn graph_lane_plot(
    ...
    theme: &Theme,
    app: &mut AppState,
    comp: &luminal_core::model::Composition,
...
fn graph_plot_retime(
    ...
    app: &mut AppState,
    comp: &luminal_core::model::Composition,
    layer: &luminal_core::model::Layer,
    retime: &luminal_core::retime::Retime,
    src_fps: f64,
    rect: egui::Rect,
) {
    ...
    let mut preset_ease: Option<luminal_core::retime::Ease> = None;
    ...
    let pct_to_speed = |pct: f64| {
        luminal_core::Rational::from_f64_on_grid(pct / 100.0, 1000)
            .unwrap_or(luminal_core::Rational::ONE)
    };
    ...
    let sampled: &luminal_core::retime::Retime = provisional.as_ref().unwrap_or(retime);
    ...
    if let Some(ease) = preset_ease {
        ...
        if let Some(new_rt) = retime.with_segment_ease(rational_at(lt.max(0.0)), ease) {
            pending = Some(luminal_core::Op::SetLayerRetime {
                comp: comp.id,
                layer: layer.id,
                retime: Some(new_rt),
            });
...
fn prop_unit(prop: luminal_core::model::TransformProp) -> &'static str {
    use luminal_core::model::TransformProp as P;
    match prop {
...
fn key_shape(k: &luminal_core::anim::Keyframe) -> KeyShape {
    use luminal_core::anim::SideInterp;
...
fn side_influence(side: luminal_core::anim::SideInterp) -> f64 {
    match side {
        luminal_core::anim::SideInterp::Bezier { influence, .. } => influence,
        _ => 1.0 / 3.0,
    }
}
...
fn side_speed(side: luminal_core::anim::SideInterp) -> Option<f64> {
    match side {
        luminal_core::anim::SideInterp::Bezier { speed, .. } => Some(speed),
        _ => None,
    }
}
...
fn apply_tangent(
    k: &mut luminal_core::anim::Keyframe,
    is_out: bool,
    speed: f64,
    influence: f64,
    mirror: bool,
    ...
    use luminal_core::anim::SideInterp::Bezier;
    if is_out {
...
fn partner_influence(
    partner: luminal_core::anim::SideInterp,
    seg: f64,
    new_speed: f64,
    sx: f64,
    sy: f64,
...
fn nudge_selected_values(
    keys: &mut [luminal_core::anim::Keyframe],
    selection: &[usize],
    delta: f64,
) {
    for &i in selection {
...
fn graph_multi_selection(
    ...
    keys: &[luminal_core::anim::Keyframe],
) -> Option<Vec<usize>> {
...
fn graph_plot(
    ...
    comp: &luminal_core::model::Composition,
    layer: &luminal_core::model::Layer,
    current: luminal_core::model::TransformProp,
    ...
    use luminal_core::anim::{Animation, Keyframe, SideInterp};
    ...
    if ui.input(|i| i.key_pressed(egui::Key::F9)) {
        set_sides = Some(luminal_core::anim::EASY_EASE);
    }
    ...
    // so sample it and grow the range to keep the whole curve on screen (Mack).
    {
        ...
        for i in 0..=n {
            ...
            let v = luminal_core::anim::evaluate(&shown, t).unwrap_or(static_val);
            vmin = vmin.min(v);
            vmax = vmax.max(v);
        }
    }
    ...
    let sample_at = |t: f64| -> f64 {
        if app.graph_speed_view {
            ...
            luminal_core::anim::evaluate_speed(&shown, t).unwrap_or(0.0)
        } else {
            luminal_core::anim::evaluate(&shown, t).unwrap_or(static_val)
        }
    };
    ...
    for (idx, key) in keys.iter().enumerate() {
        ...
        resp.context_menu(|ui| {
            ...
            if ui.button("Easy ease").clicked() {
                sides = Some(luminal_core::anim::EASY_EASE);
                ui.close_menu();
            }
    ...
    if let Some(new_keys) = pending {
        let op = if is_retime {
            ...
            match luminal_core::retime::Retime::from_source_keyframes(&new_keys) {
                Some(rt) => luminal_core::Op::SetLayerRetime {
                    comp: comp.id,
                    layer: layer_id,
                    retime: Some(rt),
                },
...
fn patch_layer_prop(
    ...
    prop: luminal_core::model::TransformProp,
    value: f64,
) -> luminal_core::model::Composition {
    let mut patched = comp.clone();
    if let Some(l) = patched.layers.iter_mut().find(|l| l.id == layer) {
        *l.transform.get_mut(prop) = luminal_core::anim::Property::fixed(value);
    }
    patched
}
...
fn build_comp_draws(
    ...
    use luminal_core::model::LayerKind;
    let in_span = |l: &luminal_core::model::Layer| {
        t_comp >= l.in_point.0.to_f64() && t_comp < l.out_point.0.to_f64()
    };
    let pixels_for = |layer: &luminal_core::model::Layer| -> Option<LayerPixels> {
        let raw = match &layer.kind {
            ...
            LayerKind::Text { document } => in_span(layer).then(|| {
                ...
                let r = luminal_text::rasterise_line(
                    &document.text,
    ...
    for layer in comp.layers.iter().rev() {
        ...
        let matte = layer.matte.as_ref().and_then(|mr| {
            ...
            Some(MatteDraw {
                ...
                luma: matches!(mr.channel, luminal_core::model::MatteChannel::Luma),
        ...
        draws.push(CompLayerDraw {
            ...
            mask_cov: match &layer.kind {
                LayerKind::Precomp { .. } if !layer.masks.is_empty() => {
                    ...
                    Some((
                        crate::export::mask_rgba(&luminal_core::mask::combined_coverage(
                            &layer.masks,
                            w,
                            h,
                            f64::from(w),
...
fn visible_control(
    ...
    if resp.on_hover_text("Show / hide this layer").clicked() {
        *pending = Some(luminal_core::Op::SetLayerVisible {
            comp: comp_id,
            layer: layer.id,
            visible: !vis,
        });
...
fn matte_control(
    ...
    use luminal_core::model::{MatteChannel, MatteRef};
    ...
    if let Some(matte) = set {
        *pending = Some(luminal_core::Op::SetLayerMatte {
            comp: comp_id,
            layer: layer.id,
            matte,
        });
...
fn blend_name(b: luminal_core::model::BlendMode) -> &'static str {
    use luminal_core::model::BlendMode;
    match b {
    ...
    bare_dropdown(
        ...
        |ui| {
            for mode in [
                ...
                {
                    if layer.blend != mode {
                        *pending = Some(luminal_core::Op::SetLayerBlend {
                            comp: comp_id,
                            layer: layer.id,
                            blend: mode,
                        });
...
fn three_d_control(
    ...
    if ui
        ...
        .clicked()
    {
        *pending = Some(luminal_core::Op::SetLayerThreeD {
            comp: comp_id,
            layer: layer.id,
            three_d: !layer.switches.three_d,
        });
...
fn mute_control(
    ...
    if resp
        ...
        .clicked()
    {
        *pending = Some(luminal_core::Op::SetLayerAudible {
            comp: comp_id,
            layer: layer.id,
            audible: muted,
        });
...
fn layer_keyframe_times(layer: &luminal_core::model::Layer) -> Vec<f64> {
    use luminal_core::anim::Animation;
    use luminal_core::model::{LayerKind, TransformProp};
...
struct RowCtx<'a> {
    theme: &'a Theme,
    comp_id: uuid::Uuid,
    layer: &'a luminal_core::model::Layer,
    lt: f64,
    off: f64,
    fps: f64,
...
fn stopwatch(
    ...
    lt: f64,
) -> Option<luminal_core::anim::Animation> {
    use luminal_core::anim::{Animation, Keyframe, SideInterp};
...
fn speed_with_key(
    retime: &Option<luminal_core::retime::Retime>,
    dur: luminal_core::Rational,
    lt: f64,
    speed: luminal_core::Rational,
) -> Option<luminal_core::retime::Retime> {
    use luminal_core::Rational;
    let mut keys = retime
        .as_ref()
    ...
    luminal_core::retime::Retime::from_speed_keyframes(Rational::ZERO, &keys)
}
...
/// editor's value lens.
#[cfg(feature = "media")]
fn layer_source_fps(app: &AppState, layer: &luminal_core::model::Layer, fallback_fps: f64) -> f64 {
    use luminal_core::model::LayerKind;
...
fn layer_source_fps(
    _app: &AppState,
    _layer: &luminal_core::model::Layer,
    fallback_fps: f64,
) -> f64 {
    fallback_fps
}
...
fn upsert_value_key(
    keys: &mut Vec<(luminal_core::Rational, luminal_core::Rational)>,
    lt: f64,
    src: f64,
    dur: luminal_core::Rational,
    comp_fps: f64,
...
fn speed_property_row(
    ...
    if speed_lens {
        ...
        if stopwatch_button(&mut c, ctx.theme, animated, hover) {
            let new_retime = if animated {
                if (current - 100.0).abs() < 1e-6 {
                    ...
                    Some(luminal_core::retime::Retime::constant_speed(
                        dur,
                        Rational::ZERO,
                        to_speed(current),
                    ))
        ...
        if stopwatch_button(&mut c, ctx.theme, time_enabled, hover) {
            let new_retime = if time_enabled {
                ...
                luminal_core::retime::Retime::from_value_keyframes(&keys)
            };
            *pending = Some(luminal_core::Op::SetLayerRetime {
                comp: ctx.comp_id,
                layer: ctx.layer.id,
            ...
            let new_retime = if speed_lens {
                if on_key {
                    let kept: Vec<(Rational, Rational)> = keys
                        ...
                        .collect();
                    luminal_core::retime::Retime::from_speed_keyframes(Rational::ZERO, &kept)
                } else {
                    speed_with_key(retime, dur, ctx.lt, to_speed(current))
                }
            } else if on_key {
                ...
                luminal_core::retime::Retime::from_value_keyframes(&kv)
            } else {
        ...
        if resp.drag_stopped() || resp.lost_focus() {
            ...
            if (new_src - src_now).abs() > 0.5 / src_fps {
                ...
                *pending = Some(luminal_core::Op::SetLayerRetime {
                    comp: ctx.comp_id,
                    layer: ctx.layer.id,
                    retime: luminal_core::retime::Retime::from_value_keyframes(&keys),
                });
            }
            c.data_mut(|d| d.remove::<f64>(id));
        }
    ...
    let track_keys: Option<Vec<luminal_core::Rational>> = if speed_lens {
        animated
    ...
    if let Some(times) = track_keys {
        let kf: Vec<luminal_core::anim::Keyframe> = times
            .iter()
            .map(|t| luminal_core::anim::Keyframe {
                time: *t,
                value: 0.0,
                interp_in: luminal_core::anim::SideInterp::Linear,
                interp_out: luminal_core::anim::SideInterp::Linear,
            })
            .collect();
...
fn mask_space(
    ...
    match &layer.kind {
        ...
        luminal_core::model::LayerKind::Adjustment => {
            (f64::from(comp.width), f64::from(comp.height))
        }
        luminal_core::model::LayerKind::Solid { def } => app
            .store
            .snapshot()
            .solid(*def)
            .map(|sd| (f64::from(sd.width), f64::from(sd.height)))
            .unwrap_or((f64::from(comp.width), f64::from(comp.height))),
        luminal_core::model::LayerKind::Precomp { comp: nested } => app
            .store
            .snapshot()
            .comp(*nested)
            .map(|n| (f64::from(n.width), f64::from(n.height)))
            .unwrap_or((f64::from(comp.width), f64::from(comp.height))),
        luminal_core::model::LayerKind::Camera { .. }
        | luminal_core::model::LayerKind::Sequence { .. }
        | luminal_core::model::LayerKind::Text { .. } => {
            (f64::from(comp.width), f64::from(comp.height))
        }
        #[cfg(feature = "media")]
        luminal_core::model::LayerKind::Footage { item, .. } => match app.media.map.get(item) {
...
#[cfg(feature = "media")]
fn blend_of(b: luminal_core::model::BlendMode) -> luminal_gpu::Blend {
    use luminal_core::model::BlendMode;
    match b {
        BlendMode::Normal => luminal_gpu::Blend::Normal,
        BlendMode::Add => luminal_gpu::Blend::Add,
        BlendMode::Multiply => luminal_gpu::Blend::Multiply,
        BlendMode::Screen => luminal_gpu::Blend::Screen,
        BlendMode::Overlay => luminal_gpu::Blend::Overlay,
        BlendMode::SoftLight => luminal_gpu::Blend::SoftLight,
        BlendMode::HardLight => luminal_gpu::Blend::HardLight,
        BlendMode::Lighten => luminal_gpu::Blend::Lighten,
        BlendMode::Darken => luminal_gpu::Blend::Darken,
    }
}

/// Layer time → rational on the flick grid (the only f64→rational route).
fn rational_at(seconds: f64) -> luminal_core::Rational {
    luminal_core::Rational::from_f64_on_grid(seconds.max(0.0), luminal_core::Rational::FLICK_DEN)
        .unwrap_or(luminal_core::Rational::ZERO)
}
...
fn upsert_key(
    ...
    lt: f64,
    value: f64,
) -> Vec<luminal_core::anim::Keyframe> {
    use luminal_core::anim::{Animation, Keyframe, SideInterp};
...
pub enum DrawSource {
    ...
    Nested {
        ...
        camera: Option<luminal_core::model::CameraPose>,
    },
}
...
pub struct GpuViewer {
    ctx: luminal_gpu::GpuContext,
    engine: luminal_gpu::ColourEngine,
    compositor: luminal_gpu::Compositor,
...
impl GpuViewer {
    pub fn new(render_state: egui_wgpu::RenderState) -> Self {
        let ctx = luminal_gpu::GpuContext::from_parts(
            render_state.device.clone(),
            render_state.queue.clone(),
        );
        let engine = luminal_gpu::ColourEngine::new(&ctx);
        let compositor = luminal_gpu::Compositor::new(&ctx);
        Self {
            ctx,
            engine,
            compositor,
    ...
    pub fn export_context(&self) -> luminal_gpu::GpuContext {
        luminal_gpu::GpuContext::from_parts(self.ctx.device.clone(), self.ctx.queue.clone())
    }
    ...
    fn realise(
        ...
        let matte_textures: Vec<Option<egui_wgpu::wgpu::Texture>> = layers
                ...
                l.matte.as_ref().map(|m| {
                    ...
                    self.compositor.composite_with_camera(
                        ...
                        &[luminal_gpu::CompositeLayer {
                            ...
                            opacity: m.opacity,
                            matte: None,
                            blend: luminal_gpu::Blend::Normal,
                            z: m.z,
            ...
            .collect();
        let comp_layers: Vec<luminal_gpu::CompositeLayer> = linear_textures
            .iter()
            .zip(layers)
            .zip(&matte_textures)
            .zip(&mask_textures)
            .map(
                |(((texture, l), matte_tex), mask_tex)| luminal_gpu::CompositeLayer {
                    texture,
                    size: l.natural_size,
                    position: l.position,
                    anchor: l.anchor,
                    ...
                    three_d: l.three_d,
                    matte: matte_tex.as_ref().map(|mt| luminal_gpu::MatteInput {
                        texture: mt,
...
impl Shell {
    ...
    fn add_mask_to_selected(&mut self, kind: ShapeKind) {
        ...
        let mask = match kind {
            ShapeKind::Rectangle => {
                luminal_core::mask::Mask::rectangle(w * 0.25, h * 0.25, w * 0.5, h * 0.5)
            }
            ShapeKind::Ellipse => {
                luminal_core::mask::Mask::ellipse(w * 0.5, h * 0.5, w * 0.3, h * 0.3)
            }
            ShapeKind::Star => {
                luminal_core::mask::Mask::star(w * 0.5, h * 0.5, w * 0.32, w * 0.14, 5)
            }
        };
        let mut masks = layer.masks.clone();
        masks.push(mask);
        self.app.commit(luminal_core::Op::SetLayerMasks {
            comp: comp_id,
            layer: layer_id,
            masks,
        });
    ...
    pub fn ui(&mut self, ctx: &egui::Context) {
                            ...
                            let patched = if let Some((sl, nx, ny)) = self.app.scale_preview {
                                ...
                                self.app
                                    ...
                                    .or_else(|| {
                                        ...
                                        let luminal_core::anim::Animation::Keyframed(keys) =
                                        ...
                                        keys.sort_by_key(|k| k.time);
                                        let lt = t_comp - layer.start_offset.0.to_f64();
                                        let value = luminal_core::anim::evaluate(&keys, lt)?;
        ...
        for &panel in floating.iter() {
            let vid = egui::ViewportId::from_hash_of(("luminal-float", panel.title()));
            let builder = egui::ViewportBuilder::default()
                .with_title(format!("Luminal — {}", panel.title()))
...
mod geometry_tests {
    ...
    use luminal_core::model::{
        Composition, Document, Layer, LayerKind, LinearColour, Switches, TransformGroup,
    };
    use luminal_core::time::{CompTime, Duration, FrameRate, Rational};
...
mod dock_tests {
    ...
    fn tangent_drag_unifies_by_default_and_alt_toggles_latched() {
        use luminal_core::anim::{Keyframe, SideInterp::Bezier, EASY_EASE};
    ...
    fn setting_key_speed_round_trips_through_the_derivative() {
        use luminal_core::anim::{evaluate, Keyframe, SideInterp};
    ...
    fn value_key_upsert_builds_a_passthrough_with_a_playhead_key() {
        use luminal_core::retime::Retime;
        use luminal_core::Rational;
    ...
    fn follow_edit_points_the_graph_at_the_touched_channel() {
        ...
        follow_edit(
            &mut app,
            &luminal_core::Op::SetLayerRetime {
                comp,
                layer,
                retime: None,
            },
    ...
    fn marquee_key(t: f64, v: f64) -> luminal_core::anim::Keyframe {
        use luminal_core::anim::{Keyframe, SideInterp};
        Keyframe {
            time: rational_at(t),
            value: v,
    ...
    fn a_selection_reads_stale_once_the_keys_change_underneath() {
        ...
        let sel = GraphSelection {
            layer: uuid::Uuid::nil(),
            prop: luminal_core::model::TransformProp::PositionX,
            retime: false,
    ...
    fn moving_a_layer_shifts_the_whole_span_not_slips_it() {
        use luminal_core::time::CompTime;
